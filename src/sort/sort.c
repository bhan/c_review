...
merge(int arr[], int lstart, int llen, int rstart, int rlen, int aux[]) {
  D printf("merge:"); D printf("larr:"); D print_array(&arr[lstart], llen);
  D printf(",rarr:"); D print_array(&arr[rstart], rlen);
  ...
  for (int i = lstart; i < lstart+llen+rlen; ++i) {
    if (lidx < lstart+llen && ridx < rstart+rlen) {
      ...
      if (arr[lidx] >= arr[ridx]) { aux[i] = arr[ridx]; ++ridx;
      } else { aux[i] = arr[lidx]; ++lidx; }
  ...
  for (int i = lstart; i < lstart+llen+rlen; ++i) arr[i] = aux[i];

  D printf("merge:result:"); D print_array(&arr[lstart], llen+rlen);
  D printf("\n");
}

inline static void
mergesort_helper(int arr[], int start, int len, int aux[]) {
  D printf("mergesort_helper:"); D printf("arr:");
  D print_array(&arr[start], len); D printf(",start:%d,len:%d\n", start, len);
...
void mergesort(int arr[], int len) {
  assert(arr != NULL); assert(len >= 0);
  D printf("mergesort:"); D print_array(arr, len); D printf(",%d\n", len);

  int aux[len];
  mergesort_helper(arr, 0, len, aux);
}

inline static int quicksort_partition(int arr[], int len) {
  int pidx = len/2; int pivot = arr[pidx];
  swap(&arr[pidx], &arr[len-1], sizeof(int));
  int swapidx = 0;
  // rearrange all elements less than pivot to be below an index: swapidx
  for (int i = 0; i < len-1; ++i) {
    if (arr[i] < pivot) {
      swap(&arr[i], &arr[swapidx], sizeof(int)); ++swapidx;
    }
  }
